...
$random_product = "select distinct
                                           p.products_id,
										   p.label_id,
                                           pd.products_name,
                                           p.products_image,
  ...
  $module_content = '';
		$products_new = vam_db_query($random_product);
...
if ($random_product['products_name'] != '') {
	...
	// set cache ID
	 if (!CacheCheck()) {
		$box->caching = 0;
		$box_whats_new = $box->fetch(CURRENT_TEMPLATE.'/boxes/box_whatsnew_multiproduct.html');
	} else {
		$box->caching = 1;
		$box->cache_lifetime = CACHE_LIFETIME;
		$box->cache_modified_check = CACHE_CHECK;
		$cache_id = $_SESSION['language'].$random_product['products_id'].$_SESSION['customers_status']['customers_status_name'];
		$box_whats_new = $box->fetch(CURRENT_TEMPLATE.'/boxes/box_whatsnew_multiproduct.html', $cache_id);
	}

	$vamTemplate->assign('box_WHATSNEW_MULTIPRODUCT', $box_whats_new);
}
?>