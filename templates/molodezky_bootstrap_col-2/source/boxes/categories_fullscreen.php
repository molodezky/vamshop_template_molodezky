...
$categories_stringfullscreen = '';


function vam_categoryfullscreen_get_category_products( $cat_id )
{

    global $categories_stringfullscreen;
    ...
    while ( $products = vam_db_fetch_array( $products_query, true ) )
    {
        ...
        $categories_stringfullscreen .= '<li class="categorie_product"><a href="' . $p_url . '">' . $products[ 'products_name' ] . '</a></li>';
...
function vam_categoryfullscreen_get_subcategory( $owner_cat_id, $owner_cat_name = '', $owner_cat_image = '', $current_cPath = '' )
{
    ...
    if ( $owner_cat_id )
    {
        ...
        $categories_stringfullscreen .= '<li><a href="' . $cPath_new_url . '">' . $owner_cat_name . $products_count_string . '</a>';
    };  // if ( $owner_cat_id )
    ...
    {
        if ( $owner_cat_id ) $categories_stringfullscreen .= '<ul>';

        foreach ( $categories_current_level as $v )
        {
            vam_categoryfullscreen_get_subcategory(
                $v[ 'id' ],
                $v[ 'name' ],
                $v[ 'image' ],
                $v[ 'path' ]
...
//        vam_categoryfullscreen_get_category_products( $owner_cat_id );
...
//    }  // if ( $categories_current_level )
    ...
    if ( $owner_cat_id ) $categories_stringfullscreen .= '</li>';
    
}  // function vam_categoryfullscreen_get_subcategory( ... )
...
if( !$box->isCached( CURRENT_TEMPLATE . '/boxes/box_categories_fullscreen.html', $cache_id ) || !$cache )
{
...
//  require_once ( DIR_FS_INC . 'vam_has_category_subcategories.inc.php' );
  ...
  vam_categoryfullscreen_get_subcategory( 0 );
  


$box->assign( 'BOX_CONTENT', $categories_stringfullscreen );
...
if ( !$cache )
{
	$box_categoriesfullscreen = $box->fetch( CURRENT_TEMPLATE . '/boxes/box_categories_fullscreen.html' );
} else {
	$box_categoriesfullscreen = $box->fetch( CURRENT_TEMPLATE . '/boxes/box_categories_fullscreen.html', $cache_id );
};

$vamTemplate->assign( 'box_CATEGORIES_FULLSCREEN', $box_categoriesfullscreen );
?>