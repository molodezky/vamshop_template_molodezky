...
/* -----------------------------------------------------------------------------------------
  ...
  $count_cur++;
    $currencies_string .= '<li class="'. $key.'"><a href="' . vam_href_link(basename($PHP_SELF), 'currency=' . $key.'&'.vam_get_all_get_params(array('language', 'currency')), $request_type) . '">' . $value['title'] . '</a></li>';
  }
