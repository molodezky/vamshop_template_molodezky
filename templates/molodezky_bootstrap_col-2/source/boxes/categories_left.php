...
function vam_category_left_get_category_products( $cat_id )
{
...
function vam_category_left_get_subcategory( $owner_cat_id, $owner_cat_name = '', $owner_cat_image = '', $current_cPath = '' )
{
    ...
    if ( $owner_cat_id )
    {
        ...
        $categories_string2 .= '<li><a href="' . $cPath_new_url . '">' . $owner_cat_name . $products_count_string . '</a>';
    };  // if ( $owner_cat_id )
    ...
    if ( $categories_current_level )
    {
        ...
        {
            vam_category_left_get_subcategory(
                $v[ 'id' ],
                $v[ 'name' ],
                $v[ 'image' ],
                $v[ 'path' ]
...
//        vam_category_left_get_category_products( $owner_cat_id );
...
}  // function vam_category_left_get_subcategory( ... )
...
//  require_once ( DIR_FS_INC . 'vam_has_category_subcategories.inc.php' );
  ...
  vam_category_left_get_subcategory( 0 );
...
if ( !$cache )
{
	$box_categories2 = $box->fetch( CURRENT_TEMPLATE . '/boxes/box_categories_left.html' );
} else {
	$box_categories2 = $box->fetch( CURRENT_TEMPLATE . '/boxes/box_categories_left.html', $cache_id );
};

$vamTemplate->assign( 'box_CATEGORIES_LEFT', $box_categories2 );
?>