...
if ($random_product["products_id"] != '') {
	// set cache ID
	 if (!CacheCheck()) {
		$box->caching = 0;
		$box_specials = $box->fetch(CURRENT_TEMPLATE.'/boxes/box_specials_multiproduct.html');
	} else {
		$box->caching = 1;
		$box->cache_lifetime = CACHE_LIFETIME;
		$box->cache_modified_check = CACHE_CHECK;
		$cache_id = $_SESSION['language'].$random_product["products_id"].$_SESSION['customers_status']['customers_status_name'];
		$box_specials = $box->fetch(CURRENT_TEMPLATE.'/boxes/box_specials_multiproduct.html', $cache_id);
	}
	$vamTemplate->assign('box_SPECIALS_MULTIPRODUCT', $box_specials);
}
?>