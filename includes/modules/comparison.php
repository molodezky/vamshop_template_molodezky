    ...
    if ( count( $product_ids_array ) > 0 ) // Customer select products
    {
        ...
        if ( vam_db_num_rows( $specifications_query ) > 0 )
        {
            ...
            foreach ( $product_ids_array as $tmp_products_id )
            {
                ...
                $module_contents .= '<div class="ProductName"><a href="'.vam_href_link(FILENAME_PRODUCT_INFO, 'products_id='.$tmp_products_id).'">'.$field_array['products_name'].'</a></div>';
                $module_contents .= '<div class="ProductPrice">' .$price .'</div>' . $buy_button;
                $module_contents .= '        </td>' . PHP_EOL;
            ...
            $remove_spec_button = '<a class="btn btn-primary remove_spec_button" href="javascript:void(0)"><i class="fa fa-times" aria-hidden="true"></i></a>';
...
$vamTemplate->assign( 'BUTTON_CONTINUE', '<a class="btn btn-primary" href="' . vam_href_link( FILENAME_DEFAULT, 'cat=' . $current_category_id ) . '">Назад</a>' );
